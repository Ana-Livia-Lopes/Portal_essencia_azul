...
class Page {
            ...
            for (let [ eventName, callbacks ] of Object.entries(events)) {
                ...
                for (const callback of callbacks) {
                    if (
                        ...
                        !(callback instanceof Event) &&
                        !(callback instanceof Event.Getter)
                    ) throw new TypeError("An event must be a function or an array of functions");
    ...
    static Asset = class AssetPage extends Page {
        ...
        process(parameters) {
            parameters.response.setHeader("Content-Type", this.contentType);
    ...
    static Rest = class RestPage extends Page {
        constructor(paths, files, { events } = {}) {
            super(paths, files, { contentType: "application/json", events });
        }
    ...
    static Router = class PageRouter extends Map {
        ...
        clear() {
            for (const page of this._pagesInstances.values()) {
                for (const path of page.paths) {
                    super.delete(path.name);
                }
            }
            this._pagesInstances.clear(ProtectedSetPermissionKey);
        }
    ...
    static {
        ...
        this._PublicTypes = Object.freeze([ "screen", "asset", "executable", "rest" ]);
