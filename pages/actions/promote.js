...
module.exports = async function execute({ request, query, session }) {
    ...
    if (nivel === "dev") throw new PermissionError(response, "Não é possível promover um administrador de maior nível");
    if (nivel === "direcao") throw new PermissionError(response, "Administrador já está no nível de direção");

    await updateDoc(docRef, { nivel: "direcao" });
