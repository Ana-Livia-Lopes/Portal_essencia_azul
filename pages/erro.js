...
async function getPageHTMLbase() {
    ...
    pageHTMLbase = (await fs.promises.readFile(path.resolve(__dirname, "./erro.html"))).toString("utf-8");
    return pageHTMLbase;
}
...
module.exports = async function execute($1, $2) {
    try {
        ...
        $1.content.clear();
        $1.content.append(`Erro fatal no processamento de erro (etapa 2), comunique a gerência do servidor: ${err}`);
    }
}